//
// Created by Paul Sweeney on 18/10/2021.
//

#ifndef REANIMATE_EXAMPLES_H
#define REANIMATE_EXAMPLES_H

extern void example_Discrete_BloodFlow(std::string file_path, std::string file_name, std::string build_path);
extern void example_Hybrid_BloodFlow();
extern void example_MicroFluidic_BloodFlow();
extern void example_BranchingOrder();
extern void example_ExtractSubnetwork();
extern void Medulla();

#endif //REANIMATE_EXAMPLES_H
